...
const Product = {
    getAll: async (request: SearchProductRequest): Promise<ApiResponse<PageListResponse<ProductResponse>>> => {
        const response = await Api.get<PageListResponse<ProductResponse>>(`/product/search-product?Search=${request.search}&Page=${request.page}&PageSize=${request.pageSize}`);
